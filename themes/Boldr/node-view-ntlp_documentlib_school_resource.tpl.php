...
//        $project_subjects .= '<a href="'.url('taxonomy/term/'.$subject->tid).'">'.$subject->name.'</a> , ';
//    }
    $i = 0;
    foreach($category_array as $subject) {
        if(empty($category_array[$i+1])) {
            $project_subjects .= '<a href="'.url('ntlp/document/library/search/'.$subject->tid).'">'.str_replace('%2C', ',', $subject->name).'</a>';
        }else {
            $project_subjects .= '<a href="'.url('ntlp/document/library/search/'.$subject->tid).'">'.str_replace('%2C', ',', $subject->name).'</a>, &nbsp;';
...
if($keywords_array != null) {
    ...
    foreach($keywords_array as $keyword) {
        ...
        if(empty($keywords_array[$i+1])) {
            $project_keywords .= '<a href="'.url('ntlp/document/library/search/'.$keyword->tid).'">'.$keyword->name.'</a>';
        }else {
            $project_keywords .= '<a href="'.url('ntlp/document/library/search/'.$keyword->tid).'">'.$keyword->name.'</a>, &nbsp;';
        }
        $i++;
...
if(isset($school_Obj)) {
    $edit_link = '<a href="'.url('ntlp/document/library/edit/resource/'.$school_Obj->school_nid.'/'.$node->nid).'">Edit</a>';
...
if($categ_Obj != false) {
    $resource_title = l($node->title, 'ntlp/document/library/resources/view/'.$node->nid.'/'.$categ_Obj->tid);
}else {
