...
<html>
<head>
<title>Google Apps - sign in</title>
</head>
<body>
<form name="samlForm" action="$requestAttr[acsURL]" method="post">
        <p><font face="Arial, sans-serif">Please wait...</font></p>
...
//	if (strpos($privateKey, '-----BEGIN DSA PRIVATE KEY-----') !== false) $xmlSecKeyType = XMLSecurityKey::DSA_SHA1;
	...
	$xmlSecDSig->insertSignature($assertion, $subject);
        ...
        global $base_path;
	$doc->save($_SERVER["DOCUMENT_ROOT"].$base_path.'sites/all/tmp/'.get_new_xmlfilename('signed.xml'));
	
	$samlResponse = $doc->saveXML();

	return $samlResponse;
}
function get_new_xmlfilename($filename) {
    $prefix = time();
