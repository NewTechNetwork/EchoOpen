...
(function ($) {
Drupal.behaviors.modalFrameSetup = function() {
    $('.modalframe-setup-child:not(.modalframe-setup-processed)').addClass('modalframe-setup-processed').click(function() {
    var element = this;
    ...
    function onSubmitCallbackSetup(args, statusMessages) {
      // Display status messages generated during submit processing.
      if (statusMessages) {
        $('.modalframe-setup-messages').hide().html(statusMessages).show('slow');
      }
    ...
    $('.modalframe-setup-messages').hide('fast');

    // Build modal frame options.
    var modalOptions = {
      url: $(element).attr('href'),
      autoFit: true,
      onSubmit: onSubmitCallbackSetup
    };

    // Try to obtain the dialog size from the className of the element.
    var regExp = /^.*modalframe-setup-size\[\s*([0-9]*\s*,\s*[0-9]*)\s*\].*$/;
