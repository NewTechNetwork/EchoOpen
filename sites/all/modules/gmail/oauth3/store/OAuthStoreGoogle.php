...
// can you include this after the reqular include?
//require_once('../../../../../config.php'); // depends where this is included from :/ :(
//$CFG->prefix

require_once 'OAuthStoreAbstract.class.php';

//$docroot =  $_SERVER['DOCUMENT_ROOT'];
//$drupalpath = $docroot.'/drupal-ahmer2/';
//chdir ($drupalpath);
//require_once $drupalpath.'includes/bootstrap.inc';
//require_once $drupalpath.'includes/common.inc';
//drupal_bootstrap(DRUPAL_BOOTSTRAP_DATABASE);


class OAuthStoreGoogle extends OAuthStoreAbstract
{
    /**
     * The MySQL connection (is no longer necessary)
     */
    protected $conn;
    ...
    /**
     * Construct the OAuthStoreMoodle.
     ...
     * @return array
     */
    public function getSecretsForSignature ( $uri, $user_id, $domain, $oauthsecret )
    {
        ...
        $secrets = array();
        if ( !$rec = db_fetch_object(db_query("SELECT * FROM {gmail_oauth_consumer_token} WHERE user_id = %d", $user_id))) {
            watchdog('gmail', 'User ['.$user_id.'] not in token table!');
            throw new OAuthException('User not in token table.');
        }

        $secrets['consumer_key']      = $domain;   // ocr_consumer_key
        $secrets['consumer_secret']   = $oauthsecret;   //ocr_consumer_secret as consumer_secret, aka oauth secret
    ...
    public function getServerTokenSecrets ( $consumer_key, $token, $token_type, $user_id )
    {
        ...
        // it has troble calling this the first time around though
        // I removed token,$token because I thinkthe new token is already stored in the db ....
        // $token_type,'token',$token,
        if ( !$rec = db_fetch_object(db_query("SELECT * FROM {gmail_oauth_consumer_token} WHERE user_id = %d", $user_id))) {
            throw new OAuthException('Problem calling record from function getServerTokenSecrets.'."'token_type',$token_type,'token',$token,'user_id',$user_id");
        }
        //
//        $CONSUMER_KEY = 'techvalleyhs.org';
//        $CONSUMER_SECRET = 'c5j/ZJn54PQd0x+rR6dZPEy+';
        $r['consumer_key']      = $consumer_key;// ocr_consumer_key
        //$r['consumer_secret']   = $CONSUMER_SECRET;  //ocr_consumer_secret as consumer_secret, aka oauth secret
        ...
        $r['server_uri']        = 'https://mail.google.com/mail/feed/atom'; // the gmail feed//ocr_server_uri as server_uri,
        $r['request_token_uri'] = 'https://www.google.com/accounts/OAuthGetRequestToken';//ocr_request_token_uri as request_token_uri,
        $r['authorize_uri']     = 'https://www.google.com/accounts/OAuthAuthorizeToken';//ocr_authorize_uri as authorize_uri,
        $r['access_token_uri']  = 'https://www.google.com/accounts/OAuthGetAccessToken';//ocr_access_token_uri as access_token_uri
// WARNING: These settings show in  2 places FIX THIS!
// TODO: FIX THIS!

         // TODO: mimic this giant query yikes
...
//        }
    ...
    public function addServerToken ( $consumer_key, $token_type, $token, $token_secret, $user_id )
    {
        ...
        // using the mdl_gmail_oauth_consumer_token table
        // form data object
        $data = NULL;

        //$data->oct_ocr_id_ref   = 1; // always google server not necssary?
        // id
        ...
        if ( $rec = db_fetch_object(db_query("SELECT * FROM {gmail_oauth_consumer_token} WHERE user_id = %d", $user_id))) {
            $data->id = $rec->id;
            db_query("DELETE FROM {gmail_oauth_consumer_token} WHERE user_id = %d", $user_id);
        }
        db_query("INSERT INTO {gmail_oauth_consumer_token} (user_id, token, token_secret, token_type, timestamp) 
            VALUES (%d, '%s', '%s', '%s', %d)",
            $user_id, $token, $token_secret, strtolower($token_type), time());
...
//                        oct_timestamp   = NOW()
//                    ',
//                    $ocr_id=1, // 1 which is always the google server but thats no
//                    $user_id,
//                    $token,
//                    $token_secret,
//                    $token_type);
...
//                ',  $consumer_key, $user_id);
        ...
        // r seems to be a mapped array with these fields
        // mdl_gmail_oauth_consumer_registry the other table we need is mdl_oauth_consumer_token
        // Hard coding the google specifics
//        $CONSUMER_KEY = 'techvalleyhs.org';
//        $CONSUMER_SECRET = 'c5j/ZJn54PQd0x+rR6dZPEy+';
        $r = array();
        ...
        // WARNING these same settings show up in 2 diff places you must set in one place OR change in both
        // TODO: FIX THIS!!!! with on location
...
//                FROM oauth_consumer_token
//                        JOIN oauth_consumer_registry
//                        ON oct_ocr_id_ref = ocr_id // this will always be 1 for google server
//                WHERE ocr_consumer_key  = \'%s\'
//                  AND oct_token         = \'%s\'
//                  AND oct_usa_id_ref    = %d
//                ', $consumer_key, $token, $user_id);
        ...
        // IF user has a token
        // count_records('gmail_oauth_consumer_token','user_id', $user_id)
        if ( $rec = get_record('gmail_oauth_consumer_token'
                 */
            $where = 'token = "'.$token.'" AND user_id = '.$user_id;
            delete_records_select('gmail_oauth_consumer_token', $where);
       // }
    }
